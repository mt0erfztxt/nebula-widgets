  ...
  (let [checked? (utils/event->checked event)]
    (rf/dispatch
      [(example010-path->keyword :set :value)
       #(if checked? (conj %1 value) (disj %1 value))])))
    ...
    (fn []
      (let [{:keys [value]} @*example040
        ...
        [example/widget
         ...
         [checkbox-group-input/widget (merge default-props {:cid "040-010" :size "large"})]
         [checkbox-group-input/widget (merge default-props {:cid "040-020"})]
         [checkbox-group-input/widget (merge default-props {:cid "ex040-03" :size "small"})]
         "```clojure
           [checkbox-group-input/widget
            {:equidistant true
             :inline true
