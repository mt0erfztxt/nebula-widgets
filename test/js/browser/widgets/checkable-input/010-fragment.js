...
test("090 It should allow get input's 'Value' part of state using '#getValuePartOfState()'", async () => {
  const sut = await getSut();
  await sut.hover();
  expect(await sut.getValuePartOfState(), 'to equal', 'Checkable input label');
});

test("100 It should allow assert on input's 'Value' part of state using '#expectValuePartOfStateIs()'", async () => {
  const sut = await getSut();
...
test("110 It should allow assert on input's 'Value' part of state using '#expectValuePartOfStateIs()' - with 'isNot' option set", async () => {
  const sut = await getSut();
